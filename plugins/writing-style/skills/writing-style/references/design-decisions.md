...
- **狀態**：有效。
- **正當性來源**：維護者 2026-09-02 定案的設計決策，來自實際回饋——2.x 各版把單份文件內部的寫法改好了，但總量沒有下降（兩次行為對照都是新版比舊版長），使用者仍覺得「很多字、很長」。診斷（Claude 與 Codex 雙路一致，Codex 修正過度推論後的講法）：既有的減法規則（刪背景、移細節、略過不適用的節）都是判斷題、沒有門檻，沒有任何規則對可見層設上限，所以規則拉扯時一律輸給「不失真」側的加法規則，總量不會下降。
- **不變條件**：文件開頭（標題之後、第一個 `##` 之前）是同步層，只放結論或現況、已決定的事、要讀者做的事、哪裡可以停；同步層有硬上限；放不進去的內容一律落到展開層（後置小節、附錄、拆檔），不得因此刪除。同步層不得出現本文件首次定義的術語。
- **可調部分**：上限數字（目前規則寫 5 個主張、hook 量 400 字元，後者約等於一分鐘的中文閱讀量，只是換算說明、不是規則命名——這是維護者採用的**實驗初始值**，不是外部共識，待用實際文件 A/B 校準：首次拿到可執行結論的時間、能否安全停讀、有沒有漏掉關鍵但書）、同步層四項的名稱與順序、展開層的排序細則。
...
- **與外部依據的關係**：硬上限的權威先例只有「主文一頁、溢出走附件」這個形狀（美軍 [AR 25-50](https://armypubs.army.mil/epubs/DR_pubs/DR_a/ARN42124-AR_25-50-007-WEB-13.pdf) §1-39b(7)：多數公文一頁、附加資訊走 enclosure；§2-8a 決策 memo 不超過兩頁）；Anthropic 對模型寫到磁碟的文件給的是校準句而非數字（[Prompting Claude Opus 5](https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/prompting-claude-opus-5)「Written deliverable length」）。兩者支持「同步層有上限、全文不設上限」，不決定數字。私人企業寫作文化（Basecamp、GitLab、Stripe）查無字數上限，數字是本地決策。LIFEBench（[arXiv 2505.16234](https://arxiv.org/abs/2505.16234)）顯示模型對短長度指示跟得上、長的急遽劣化，支持把數字放在小的同步層而非全文。**未知**：沒有研究量「為達標砍掉的是填充還是實質」，這是本條最可能的失敗模式，靠 WS-M01 與仲裁表「限定語跟著主張走」防守。
- **執行層**：可機械判定的規則靠 prompt 守不穩（2026-08-11 對照：有 hook 的規則零違規、沒 hook 的七處），所以同步層字元數與符號串接由 `scripts/check-doc-shape-hook.sh` 機械檢查；只警示、不阻擋、不截斷。語意層的判斷（哪個主張可以移、哪個但書必須留）留在 prompt，hook 量不到。已知風險：硬上限會 Goodhart——用多層子句把主張塞進一句以通過字數檢查；防守靠「一個項目一個完整主張」與同步層驗收列的明文禁止。
- **日期**：2026-09-02。
